...
def get_table(league: int = 1):
    games = pl.from_records(list(Game.objects
                                 .filter(score_team1__isnull=False)
                                 .filter(score_team2__isnull=False).values('id', 'score_team1', 'score_team2'))).rename(
        {'id': 'game_id',
         'score_team1': 'final_score_team1',
         'score_team2': 'final_score_team2'}
    )

    bets = pl.from_records(list(Bet.objects
                                .filter(league=league)
                                .filter(game__in=list(games.select('game_id').to_series()))
                                .values('id', 'game_id','league', 'user', 'score_team1', 'score_team2'))).rename({
        'id':'bet_id',
        'league':'league_id',
        'user': 'user_id',
